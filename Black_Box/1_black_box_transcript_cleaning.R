...
H2.1_bb_combined_variables <- c('rw.rLSM.patient', 'rw.rLSM.pcp', 'prov_id', 'p_mainrace', 'LSM_function_mean', 'pcp_cc_score')

H2.1_df <- blackbox_lsm_all_matching %>%
  as_tibble() %>% 
  select(H2.1_bb_combined_variables) %>%
...
H2.2a_bb_combined_variables <- c('rw.rLSM.patient', 'rw.rLSM.pcp', 'pt_cahps_1_20_score_high', 'prov_id', 'p_mainrace', 'LSM_function_mean')

H2.2a_df <- blackbox_lsm_all_matching %>%
  as_tibble() %>% 
  select(H2.2a_bb_combined_variables) %>%
  mutate(p_mainrace = factor(p_mainrace)) %>%
  tidyr::drop_na()
...
H2.2b_bb_combined_variables <- c('rw.rLSM.patient', 'rw.rLSM.pcp', 'pt_cc_score_high', 'prov_id', 'p_mainrace', 'LSM_function_mean')

H2.2b_df <- blackbox_lsm_all_matching %>%
  as_tibble() %>% 
  select(H2.2b_bb_combined_variables) %>%
  mutate(p_mainrace = factor(p_mainrace)) %>%
  mutate(prov_id= as.character(prov_id)) %>%
  tidyr::drop_na()
...
H2.2c_bb_combined_variables <- c('rw.rLSM.patient', 'rw.rLSM.pcp', 'pt_trust_score_high', 'prov_id', 'p_mainrace', 'LSM_function_mean')

H2.2c_df <- blackbox_lsm_all_matching %>%
  as_tibble() %>% 
  select(H2.2c_bb_combined_variables) %>%
  mutate(p_mainrace = factor(p_mainrace)) %>%
  mutate(prov_id= as.character(prov_id)) %>%
  tidyr::drop_na()
...
H2.2d_bb_combined_variables <- c('rw.rLSM.patient', 'rw.rLSM.pcp', 'pt_respect_score_high', 'prov_id', 'p_mainrace', 'LSM_function_mean')

H2.2d_df <- blackbox_lsm_all_matching %>%
  as_tibble() %>% 
  select(H2.2d_bb_combined_variables) %>%
  mutate(p_mainrace = factor(p_mainrace)) %>%
  mutate(prov_id= as.character(prov_id)) %>%
  tidyr::drop_na()
