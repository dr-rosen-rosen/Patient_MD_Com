...
cs_gm_df <- read.csv(here('/Users/mrosen44/Johns\ Hopkins/Salar\ Khaleghzadegan\ -\ Patient\ _Provider_Communication_Projects/cross_study_analyses/combined_smoothed_rw.rLSM.csv'))
table(cs_gm_df$Speaker)
...
# cs_gm_df %>% group_by(File) %>% summarize(n = n()) %>% min()
...
cs_gm_df3 <- cs_gm_df %>%
  ...
  ungroup() %>%
  left_join(unique(cs_gm_df[c('pcp_id','File')]), by = 'File',relationship = "many-to-many") %>%
  pivot_wider(
    # id_cols = File, 
    values_from = rLSM, 
    names_from = t,
...
print(length(unique(cs_gm_df4$pcp_id)))
...
lapply(1:10, function(k)
{
  ...
  lca_enum <- MplusAutomation::mplusObject(
    ...
    MODEL = glue::glue(
      '%OVERALL%\n',
      'i s | rLSM_t0@0 rLSM_t1@1 rLSM_t2@2 rLSM_t3@3 rLSM_t4@4\n',
    ...
    # PLOT = "type = plot3;",
    usevariables = colnames(cs_gm_df4),#'rLSM_t0 rLSM_t1 rLSM_t2 rLSM_t3 rLSM_t4 rLSM_t5 rLSM_t6 rLSM_t7 rLSM_t8 rLSM_t9 tot_wc',
    rdata = cs_gm_df4
  )
  
  ml_lpa_fit <- MplusAutomation::mplusModeler(lca_enum,
                                              dataout = glue::glue("cross_study_analysis/mplus/gmm/wickrama/lcga/LCGA_{k}.dat"),
                                              modelout = glue::glue("cross_study_analysis/mplus/gmm/wickrama/lcga/LCGA_{k}.inp"),
...
lapply(1:10, function(k)
{
  ...
  ml_lpa_fit <- MplusAutomation::mplusModeler(lca_enum,
                                              dataout = glue::glue("cross_study_analysis/mplus/gmm/wickrama/gmm_10/GMM_{k}.dat"),
                                              modelout = glue::glue("cross_study_analysis/mplus/gmm/wickrama/gmm_10/GMM_{k}.inp"),
                                              check = TRUE, run = TRUE, hashfilename = FALSE)
})
beepr::beep()

output_enum <- readModels(here("cross_study_analysis/mplus/gmm/wickrama/gmm_10"), quiet = TRUE)
...
ml_lpa2 <- lapply(1:5, function(j) # L2 profiles
{
  ...
  lca_enum <- MplusAutomation::mplusObject(
    ...
    VARIABLE = glue::glue("IDVARIABLE IS row_num;\n",
                          "CLASSES = BC({j}) c({k});\n",
                          "CLUSTER IS pcp_id;\n",
                          "WITHIN ARE rLSM_t0 rLSM_t1 rLSM_t2 rLSM_t3\nrLSM_t4 rLSM_t5 rLSM_t6 rLSM_t7 rLSM_t8 rLSM_t9;",
                          "BETWEEN ARE BC;\n"),
    # DEFINE = "STANDARDIZE rLSM_t0 rLSM_t1 rLSM_t2 rLSM_t3;",
    DEFINE = "STANDARDIZE rLSM_t0 rLSM_t1 rLSM_t2 rLSM_t3 rLSM_t4\nrLSM_t5 rLSM_t6 rLSM_t7 rLSM_t8 rLSM_t9;",
    ...
    SAVEDATA = glue::glue("FILE IS L2_{j}_L1_{k}_out.dat;\n",
               ...
               "save = CPROBABILITIES;\n",
               "TECH4 IS tech4.dat;\n"),
    usevariables = colnames(cs_gm_df4),
    rdata = cs_gm_df4
  )
  
  ml_lpa_fit <- MplusAutomation::mplusModeler(lca_enum,
                                              dataout = glue::glue("cross_study_analysis/mplus/gmm/wickrama/ml_gmm_10/L2_{j}_L1_{k}.dat"),
                                              modelout = glue::glue("cross_study_analysis/mplus/gmm/wickrama/ml_gmm_10/L2_{j}_L1_{k}.inp"),
                                              check = TRUE, run = TRUE, hashfilename = FALSE)
})
beepr::beep()
skimr::skim(cs_gm_df4)


output_enum <- readModels(here("cross_study_analysis/mplus/gmm/wickrama/ml_gmm_10"), quiet = TRUE)
enum_summary <- LatexSummaryTable(output_enum,
                                  keepCols=c("Title",
                                             "LL",
                                             "BIC",
...
L1 <- 3
L2 <- 2
out_f_dir <- 'mplus/gmm/wickrama/ml_gmm_10'
mlLPA_results <- MplusAutomation::readModels(glue::glue("cross_study_analysis/{out_f_dir}/L2_{L2}_L1_{L1}.out"), what="savedata")$savedata
...
mlLPA_results %>%
  ...
  mutate(l1 = as.factor(l1),
         ...
         variable = ordered(variable, levels = c("rlsm_t0", "rlsm_t1","rlsm_t2","rlsm_t3","rlsm_t4",
                                                 "rlsm_t5","rlsm_t6","rlsm_t7","rlsm_t8","rlsm_t9"))) %>%
...
mlLPA_results %>%
  rename('L1' = 'C',
         'L2' = 'BC') %>%
  janitor::clean_names() %>%
  full_join(cs_gm_df3, by = 'row_num') %>%
