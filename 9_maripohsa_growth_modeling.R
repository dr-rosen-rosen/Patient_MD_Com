...
M_smoothed_rLSM <- read.csv(here('MARIPOHSA','maripohsa_smoothed_rLSM_notaggregated.csv'))

############# rLSM.D models

gm_df <- M_smoothed_rLSM %>% # This df comes from runing the rLSM script trhough line 124 (stops before aggregating to file level)
...
m2.rLSM.D.MARIPOHSA <- lcmm::hlme(
  ...
  subject = 'group_id',
  data = gm_df,
  B = random(m1.rLSM.D.MARIPOHSA)
  )

# m2g.rLSM.D.MARIPOHSA <- lcmm::gridsearch(
#   rep = 100, maxiter = 30, minit = m1.rLSM.D.MARIPOHSA, 
#   hlme(rLSM ~ t,
#     random = ~ 1 + t,
#     mixture = ~ 1 + t,
#     ng = 2,
...
# m3g.rLSM.D.MARIPOHSA <- lcmm::gridsearch(
#   rep = 100, maxiter = 30, minit = m1.rLSM.D.MARIPOHSA, 
#   hlme(rLSM ~ t,
#        random = ~ 1 + t,
#        mixture = ~ 1 + t,
...
# m4g.rLSM.D.MARIPOHSA <- lcmm::gridsearch(
#   rep = 100, maxiter = 30, minit = m1.rLSM.D.MARIPOHSA,
#   hlme(rLSM ~ t,
#        random = ~ 1 + t,
#        mixture = ~ 1 + t,
...
# m5g.rLSM.D.MARIPOHSA <- lcmm::gridsearch(
#   rep = 100, maxiter = 30, minit = m1.rLSM.D.MARIPOHSA,
#   hlme(rLSM ~ t,
#        random = ~ 1 + t,
#        mixture = ~ 1 + t,
...
summarytable(
  m1.rLSM.D.MARIPOHSA,m2.rLSM.D.MARIPOHSA,m3.rLSM.D.MARIPOHSA,m4.rLSM.D.MARIPOHSA,m5.rLSM.D.MARIPOHSA,
  # m1.rLSM.P,m2.rLSM.P,m3.rLSM.P,m4.rLSM.P,m5.rLSM.P,
             which = c("G", "loglik", "AIC", "BIC", "%class")) %>% 
  as.data.frame() %>%
  write_csv('gmmTableD.MARIPOHSA.csv')
...
f.class.df <- m2.rLSM.D.MARIPOHSA$pprob %>%
  full_join(gm_df, by = 'group_id') %>%
  mutate(t = t+1) %>%
  mutate(class = recode(class,`1` = "Stable_ish", `2` = "Increasing"))

stable_mean <- round(mean(f.class.df[which(f.class.df$class == 'Stable_ish'),'rLSM']), digits = 2)
...
f.class <- f.class.df %>%
  ...
  # annotate("text", x = 8.5, y = stable_mean+.006, label = 'Overall mean rw.rLSM for stable-ish class', size = 3.5,color = "#377EB8") +
...
case_control_df <- read.csv(here('MARIPOHSA','maripohsa_case_control.csv')) %>%
  left_join(f.class.df, by = 'File') %>%
  select(File,Pros_Rating_Cases_Control,class) %>%
  distinct() %>%
  drop_na()

table(case_control_df$class,case_control_df$Pros_Rating_Cases_Control)
chisq.test(case_control_df$class,case_control_df$Pros_Rating_Cases_Control)
