...
mar_gm_df <- read.csv(here('/Users/mrosen44/Johns\ Hopkins/Salar\ Khaleghzadegan\ -\ Patient\ _Provider_Communication_Projects/cross_study_analyses/combined_smoothed_rw.rLSM.csv')) %>%
  filter(study == 'MAR')

table(mar_gm_df$Speaker)
...
# mar_gm_df %>% group_by(File) %>% summarize(n = n()) %>% min()
...
mar_gm_df3 <- mar_gm_df %>%
  ...
  ungroup() %>%
  left_join(unique(mar_gm_df[c('pcp_id','File')]), by = 'File',relationship = "many-to-many") %>%
  pivot_wider(
    # id_cols = File, 
    values_from = rLSM, 
    names_from = t,
...
skimr::skim(mar_gm_df3)
skimr::skim(demo_df)


mar_gm_df4 <- mar_gm_df3 %>% 
...
print(length(unique(mar_gm_df4$pcp_id)))

# mar_gm_df4 <- mar_gm_df4 %>% 
...
lapply(1:10, function(k)
{
  ...
  lca_enum <- MplusAutomation::mplusObject(
    ...
    # PLOT = "type = plot3;",
    usevariables = colnames(mar_gm_df4),#'rLSM_t0 rLSM_t1 rLSM_t2 rLSM_t3 rLSM_t4 rLSM_t5 rLSM_t6 rLSM_t7 rLSM_t8 rLSM_t9 tot_wc',
    rdata = mar_gm_df4
  )
  
  ml_lpa_fit <- MplusAutomation::mplusModeler(lca_enum,
                                              dataout = glue::glue("MARIPOHSA/mplus/gmm/wickrama/lcga/LCGA_{k}.dat"),
                                              modelout = glue::glue("MARIPOHSA/mplus/gmm/wickrama/lcga/LCGA_{k}.inp"),
...
lapply(1:10, function(k)
{
  ...
  ml_lpa_fit <- MplusAutomation::mplusModeler(lca_enum,
                                              dataout = glue::glue("MARIPOHSA/mplus/gmm/wickrama/gmm_4/GMM_{k}.dat"),
                                              modelout = glue::glue("MARIPOHSA/mplus/gmm/wickrama/gmm_4/GMM_{k}.inp"),
                                              check = TRUE, run = TRUE, hashfilename = FALSE)
})
beepr::beep()

output_enum <- readModels(here("MARIPOHSA/mplus/gmm/wickrama/gmm_4"), quiet = TRUE)
...
ml_lpa2 <- lapply(1:5, function(j) # L2 profiles
{
  ...
  lca_enum <- MplusAutomation::mplusObject(
    ...
    SAVEDATA = glue::glue("FILE IS L2_{j}_L1_{k}_out.dat;\n",
               ...
               "save = CPROBABILITIES;\n",
               "TECH4 IS tech4.dat;\n"),
    usevariables = colnames(mar_gm_df4),
    rdata = mar_gm_df4
  )
  
  ml_lpa_fit <- MplusAutomation::mplusModeler(lca_enum,
                                              dataout = glue::glue("MARIPOHSA/mplus/gmm/wickrama/ml_gmm_4/L2_{j}_L1_{k}.dat"),
                                              modelout = glue::glue("MARIPOHSA/mplus/gmm/wickrama/ml_gmm_4/L2_{j}_L1_{k}.inp"),
                                              check = TRUE, run = TRUE, hashfilename = FALSE)
})
beepr::beep()


output_enum <- readModels(here("MARIPOHSA/mplus/gmm/wickrama/ml_gmm_4"), quiet = TRUE)
enum_summary <- LatexSummaryTable(output_enum,
                                  keepCols=c("Title",
                                             "LL",
                                             "BIC",
...
mlLPA_results <- MplusAutomation::readModels(glue::glue("MARIPOHSA/{out_f_dir}/L2_{L2}_L1_{L1}.out"), what="savedata")$savedata
...
mlLPA_results %>%
  rename('L1' = 'C',
         'L2' = 'BC') %>%
  janitor::clean_names() %>%
  full_join(mar_gm_df3, by = 'row_num') %>%
