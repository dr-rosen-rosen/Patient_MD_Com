...
m.1_H1.1_HLM <- lmer(rLSM.D.scaled ~ 1 + (1|provider_id), 
                     data = H1.1_df)
...
sjPlot::tab_model(m.2_H1.1_HLM_conv, m.2_H1.1_HLM_rLSM.D, m.2_H1.1_HLM_rLSM.P)
...
m.0_H3a.1_HLM <- glm(
                      provcommhigh~ 1, 
                     # provcomm_clarityhigh~ 1,
                     # provcomm_responsivehigh~ 1,
                     # provcomm_expconditionhigh~ 1,
                     # provcomm_expselfcarehigh~ 1,
                     # provcomm_empowermenthigh~ 1,
                     family= 'binomial', data = H3a.1_df)
...
sjPlot::tab_model(m.1_H3a.1_HLM)
...
predictors <- append(cntrl_vars,coef_list.provcomm.matching)
predictors <- coef_list.provcomm.matching
#predictors <- append(cntrl_vars,coef_list.provcomm.speech)

predictors <- c("LSM_function_mean.scaled")
predictors <- c("LSM_function_mean.scaled", "site_id")
...
m.10_H3a.1_HLM_conv <- update(m.0_H3a.1_HLM, paste(".~. +",paste(predictors,collapse = ' + ')))
sjPlot::tab_model(m.10_H3a.1_HLM_conv)
predictors <- c('percept_D.scaled', "racecat2")
m.10_H3a.1_HLM_conv <- update(m.0_H3a.1_HLM, paste(".~. +",paste(predictors,collapse = ' + ')))
sjPlot::tab_model(m.10_H3a.1_HLM_conv)
...
m.1_H3a.2_HLM <- glm(overcommhigh ~ 1, family= 'binomial' , data = H3a.2_df)
# test for fit of grouping structure
...
m.0_H3a.3_HLM <- glm(
                      ipstylehigh ~ 1,
                     # ipstyle_friendhigh ~ 1,
                     # ipstyle_respecthigh ~ 1,
                     # ipstyle_discrimhigh ~ 1,
                     #ipstyle_emosupporthigh ~ 1,
                      family = 'binomial', data = H3a.3_df)
...
#predictors <- append(cntrl_vars,coef_list.ipstyle.matching)
predictors <- coef_list.ipstyle.matching
#predictors <- append(cntrl_vars,coef_list.ipstyle.speech)

predictors <- c("WC_sum.D.scaled", "site_id")
predictors <- c("LSM_function_mean.scaled", "site_id")
predictors <- c("informal_D.scaled", "site_id", "racecat2")
...
#backward elimination...
m.10_H3a.3_HLM_conv <- update(m.10_H3a.3_HLM_conv, ".~. -conv.affect.match.scaled")
...
#backward elimination...
m.10_H3a.3_HLM_conv <- update(m.10_H3a.3_HLM_conv, ".~. -feel_P.scaled")
...
# step 1: Null model
m.0_H3a.6_HLM <- glm(overallsat~ 1, family = 'binomial', data = H3a.6_df)
...
m.10_H4_HLM_conv <- update(m.0_H4_HLM, paste(".~. +",paste(predictors,collapse = ' + ')))
sjPlot::tab_model(m.10_H4_HLM_conv)
predictors <- c('conv.relativ.match.scaled', "site_id", "racecat2")
m.10_H4_HLM_conv <- update(m.0_H4_HLM, paste(".~. +",paste(predictors,collapse = ' + ')))
sjPlot::tab_model(m.10_H4_HLM_conv)
